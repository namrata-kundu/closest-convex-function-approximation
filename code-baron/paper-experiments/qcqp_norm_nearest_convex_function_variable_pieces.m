function [rho_vals,new_pieces, sol, objective] = qcqp_norm_nearest_convex_function_variable_pieces(f,pieces)
    yalmip('clear')
...
%         num_of_pieces = ceil(right_bound - left_bound)*granularity_of_pieces;
    num_of_pieces = granularity_of_pieces;
...
%     end
%     
% Assuming new_f, new_pieces, a, b, and c are defined elsewhere in your code
...
% %     cf = new_f(3,i);
% %     
% %     % Assuming a, b, and c are numeric arrays and not requiring symbolic processing
% %     % Define a numeric function for the difference
% %     func_diff = @(x) abs((af*x.^2 + bf*x + cf) - (a(i)*x.^2 + b(i)*x + c(i)));
% %     
% %     lower_bound = new_pieces(i);
% %     upper_bound = new_pieces(i+1);
% %     
% %     % Sample points and evaluate the maximum absolute difference
% %     num_points = 1000;
% %     x_values = linspace(lower_bound, upper_bound, num_points);
% %     diffs = arrayfun(func_diff, x_values);
% %     max_diff = max(diffs);
% %     
% %     objective = objective + max_diff;
% % end
% Assuming setup for new_f, new_pieces, and optimization variables a, b, c is done elsewhere

% Define the decision variables for each piece, assumed to be done elsewhere:
% e.g., a = sdpvar(size(new_f,2),1), b = sdpvar(size(new_f,2),1), c = sdpvar(size(new_f,2),1)
    Constraints = [];
...
for i=1:size(new_f,2)
    ...
    for j = 1:num_points
        ...
        Constraints = [Constraints, max_diff_var >= diff_expr, max_diff_var >= -1*diff_expr]; % Ensure max_diff_var is greater than or equal to the abs difference at x
    end
% %         x = lower_bound;
% %         diff_expr = abs((af*x^2 + bf*x + cf) - (a(i)*x^2 + b(i)*x + c(i)));
% %         diff_expr = (((af*x^2 + bf*x + cf) - (a(i)*x^2 + b(i)*x + c(i)))^2)^(0.5);
% %         % Add constraint for this point
% %         Constraints = [Constraints, max_diff_var >= diff_expr]; % Ensure max_diff_var is greater than or equal to the abs difference at x
% % 
% %          x = (lower_bound+upper_bound)/4;
% %         diff_expr = abs((af*x^2 + bf*x + cf) - (a(i)*x^2 + b(i)*x + c(i)));
% %         diff_expr = (((af*x^2 + bf*x + cf) - (a(i)*x^2 + b(i)*x + c(i)))^2)^(0.5);
% %         % Add constraint for this point
...
% The variable 'objective' now represents the sum of maximum differences (approximation of the infinity norm)
...
%         Constraints1 = [Constraints1, con1] ;
%     end

    epsilon = 0.00005;
    for i=2:size(new_pieces,2)-1
        con1= new_pieces(i) <= (new_pieces(i+1)-epsilon);
%         Constraints1 = [Constraints1, con1]:'constraints1';
        Constraints1 = [Constraints1, new_pieces(i-1) <= (new_pieces(i)-epsilon), new_pieces(i) <= (new_pieces(i+1)-epsilon)] ;
    end
  ...
  Constraints = [Constraints, x2 == x_val*x_val];
        ...
        Constraints3 = [Constraints3, ai_val*x2 + bi_val*x_val + ci_val == aiplus1_val*x2 + biplus1_val*x_val + ciplus1_val ]:'constraints3';
     end
    ...
    for i=1:size(new_pieces,2)
        if class(new_pieces(i)) == 'sdpvar'
            assign(new_pieces(i),0);
    %         Constraints = [Constraints,left_boundary<= new_pieces(i),new_pieces(i) <=right_boundary];
        end
    end
    ...
    for i=1:size(rho,1)
        for j=1:size(rho,2)
            if class(rho(i,j)) == 'sdpvar'
                assign(rho(i,j),0);
    %             Constraints = [Constraints,-50<= rho(i,j), rho(i,j) <=50];
            end
        end
    end  
    
%     Constraints=[Constraints, Constraints1, Constraints2,Constraints3, Constraints4, Constraints5,Constraints6];%,  Constraints7,Constraints8, Constraints9, Constraints10];
    % check(Constraints)
%     allvars=allvariables(objective)
...
%          options = sdpsettings('solver','gurobi','verbose', 2,'debug',1);
    ...
    sol = optimize([Constraints, Constraints1, Constraints2,Constraints3, Constraints4, Constraints5,Constraints6],objective, options)
