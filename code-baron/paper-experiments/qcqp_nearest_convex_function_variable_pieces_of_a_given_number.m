function [rho_vals,new_pieces, sol, objective] = qcqp_nearest_convex_function_variable_pieces_of_a_given_number(f,pieces)
    yalmip('clear')
...
%         objective = objective + integral;
        ...
        Constraints = [Constraints, aa == (a_f - a_var)^2];
        Constraints = [Constraints, cc == (c_f - c_var)^2];
        Constraints = [Constraints, bb == (b_f - b_var)^2];
        
        Constraints = [Constraints, u2 == u_b^2];
        Constraints = [Constraints, l2 == l_b^2];
        
        Constraints = [Constraints, u3 == u2*u_b];
        Constraints = [Constraints, l3 == l2*l_b];
        
        Constraints = [Constraints, u4 == u2^2];
        Constraints = [Constraints, l4 == l2^2];
        
        Constraints = [Constraints, u5 == u4*u_b];
        Constraints = [Constraints, l5 == l4*l_b];
        
        Constraints = [Constraints, ac == (a_f - a_var)*(c_f - c_var)];
        Constraints = [Constraints, ab == (a_f - a_var)*(b_f - b_var)];
        Constraints = [Constraints, bc == (b_f - b_var)*(c_f - c_var)];

        objective = objective + ((u5*aa)/5 - (l5*aa)/5 - l3*((2*ac)/3 + bb/3) + u3*((2*ac)/3 + bb/3) - l_b*cc  + u_b*cc - (l4*ab)/2 - l2*bc + (u4*ab)/2 + u2*bc);
...
%     Constraints = [Constraints, -10000000<=allvars<=100000];
    ...
    sol = optimize([Constraints, Constraints1, Constraints2,Constraints3, Constraints4, Constraints5,Constraints6,Constraints7],objective, options)
