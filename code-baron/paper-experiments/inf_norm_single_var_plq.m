...
num_points = 200; % Number of points to discretize each interval
for i=1:size(new_f,2)
    syms x a_var b_var c_var
    af = new_f(1,i);
    bf = new_f(2,i);
    cf = new_f(3,i);
    h  = abs((af*x*x + bf*x + cf) - (a_var*x*x + b_var*x + c_var));
    h_prime = diff(h, x);


%     str_h_prime = char(h_prime);
%     str_h_prime = strrep(str_h_prime, 'a_var', strcat('a{',num2str(i),'}'));
%     str_h_prime = strrep(str_h_prime, 'b_var', strcat('b{',num2str(i),'}'));
%     str_h_prime = strrep(str_h_prime, 'c_var', strcat('c{',num2str(i),'}'));
% h_prime=eval(str_h_prime);
...
boundary_values = ([subs(h, x, lower_bound), subs(h, x, upper_bound)]);
...
str_infinity_norm = char(infinity_norm);
    str_infinity_norm = strrep(str_infinity_norm, 'a_var', strcat('a{',num2str(i),'}'));
    str_infinity_norm = strrep(str_infinity_norm, 'b_var', strcat('b{',num2str(i),'}'));
    str_infinity_norm = strrep(str_infinity_norm, 'c_var', strcat('c{',num2str(i),'}'));
infinity_norm = double(str_infinity_norm);
