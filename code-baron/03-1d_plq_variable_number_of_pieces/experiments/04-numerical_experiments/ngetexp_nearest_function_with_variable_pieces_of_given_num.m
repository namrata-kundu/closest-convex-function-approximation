...
function  [g, g_pieces, objective,sol] = ngetexp_nearest_function_with_variable_pieces_of_given_num(f, pieces, num_of_pieces)
%     yalmip('clear')
...
%     bounds = [left_boundary,left_boundary+0.0005];
%     for i=2:size(pieces,2)-1
%         bounds = [bounds, pieces(i)];
%     end
%     bounds = [bounds,right_boundary-0.0005, right_boundary ];


%     bounds = [left_boundary,left_boundary+0.0005, pieces(2,-1),right_boundary-0.0005, right_boundary ]
...
%         g_pieces = [left_boundary, bounds(2), sdpvar(1,num_of_pieces-1,'full'), bounds(end-1),  right_boundary];
        g_pieces = [left_boundary, left_boundary+0.0005, sdpvar(1,num_of_pieces-1,'full'),right_boundary-0.0005, right_boundary];
        g_pieces = [left_boundary,   sdpvar(1,num_of_pieces-1,'full'),  right_boundary];
    end
...
%     end
    ...
    for i=1:size(g_pieces,2)-2 %check 1 or 2
        ...
        Constraints = [Constraints, (ai_val*x_val*x_val + bi_val*x_val + ci_val == aiplus1_val*x_val*x_val + biplus1_val*x_val + ciplus1_val):'1' ];
    end
...
%     end
    ...
    for i=1:size(g_pieces,2)-2 %check 1 or 2
        ...
        Constraints = [Constraints, (2*ai_val*x_val + bi_val == 2*aiplus1_val*x_val + biplus1_val):'2' ];
    end
...
%                 Constraints = [Constraints, implies(d1(k), sum(coeffs(i:(i+f_size_block-1)))==0)];
%                 Constraints = [Constraints, implies(d1(i), sum(coeffs(i:(i+f_size_block)))<=1)];
             Constraints = [Constraints, (implies(d1(i), coeffs(i)==1 & sum(coeffs(i+1:(i+f_size_block)))==0 & coeffs(i+f_size_block+1)==1)):'4'];
        end
        Constraints = [Constraints, (sum(d1) == size(g,2)-1):'5'];
...
%         end
        ...
        for i=1:size(coeffs,2)-f_size_block
                Constraints = [Constraints, (implies(d2(i), max_ones_in_each_ff_size_block >=sum(coeffs(i:(i+f_size_block)))>=1)):'6'];
        end
%                     Constraints = [Constraints, sum(d2) == f_size_block];
        Constraints = [Constraints, (sum(d2) == size(coeffs,2)-f_size_block):'7'];

        Constraints = [Constraints, (coeffs(1)==1):'8', (coeffs(end)==1):'9'];
...
%                 Constraints = [Constraints, ismember(g_pieces(i),bounds(3:end-2))];
                ...
                for k=1:size(bounds,2)-2
                    Constraints = [Constraints, (implies(d3(k), g_pieces(i)==bounds(1+k))):'10'];
                end
                Constraints = [Constraints, (sum(d3) == 1):'11'];
...
%     end
    ...
    Constraints = [Constraints, (sum(coeffs) == (size(bounds,2)-1 )):'12'];
...
% % %     end
    %'baron.maxtime',600, 
       options = sdpsettings('solver','baron', 'baron.maxtime',300,'verbose',1);
%        options = sdpsettings('solver','baron',  'verbose',1);
%         options = sdpsettings('solver','baron',  'baron.maxiter',5000, 'baron.AbsConFeasTol', 1e-10, 'baron.EpsA', 1e-10,'baron.EpsR', 1e-10, 'baron.DeltaA',1e-10, 'verbose',1,'threads',14);
% % %         options = sdpsettings('solver','baron', 'baron.maxtime',300,'baron.AbsConFeasTol', 1e-2 , 'baron.EpsA', 1e-2 ,'baron.EpsR', 1e-2 , 'baron.DeltaA',1e-2 , 'verbose',1,'threads',14);
%         options = sdpsettings('solver','baron',  'baron.maxiter',5000, 'baron.AbsConFeasTol', 1e-1 , 'baron.EpsA', 1e-1 ,'baron.EpsR', 1e-1 , 'baron.DeltaA',1e-1 , 'verbose',1,'threads',14);
%         options = sdpsettings('solver','baron',  'baron.maxiter',5000,  'verbose',1,'threads',14);
...
%       options = sdpsettings('solver','bmibnb', 'bmibnb.maxtime',1500, 'bmibnb.eps',1e-2, 'verbose',1);
