...
% Select example - uncomment one:
[pieces, f] = w_function_examples(1);           % Standard W function
% [pieces, f] = convex_function_examples(1);      % Standard convex function
% [pieces, f] = linear_and_absolute_examples(1);  % Absolute value function
% [pieces, f] = linear_and_absolute_examples(5);  % Extended piecewise linear

% For more examples, see ../examples/run_examples.m
...
%  options = sdpsettings('solver','baron'  ,'verbose', 2, 'usex0',1,'debug',1);
...
%Feed into solver
 ...
 Constraints_more = [0<=rho(1,:)<=2, f(2,1)<=rho(2,:)<=f(2,end), c_lb<=rho(3,:)<=y_upper_bound];
 Constraints_more = [0<=rho(1,:)<=max(f(1,:)), min(f(2,:))<=rho(2,:)<=max(f(2,:)), min(f(3,:))<=rho(3,:)<=y_upper_bound];
 Constraints_more = [-100<=rho(1,:)<=max(f(1,:))+100, min(f(2,:))-100<=rho(2,:)<=max(f(2,:))+100, min(f(3,:))-100<=rho(3,:)<=y_upper_bound+100];
% sol = optimize([Constraints1, Constraints2,Constraints3, Constraints4, Constraints5,Constraints6,Constraints7, -34<=UsedInObjective<=34],objective, options)
